    ...
    <div class="main-content">
      ...
      <div class="user-panel">
        ...
        <div class="question-status">
          <h3>当前题目状态</h3>
          <p v-if="currentPhase === 'question'">面试官提问中...</p>
          <p v-else-if="currentPhase === 'answer'">
            ...
            <span class="transcribing-indicator" v-if="isTranscribing">📝 转换中</span>
          </p>
        ...
        <!-- 图像采集状态 - 独立显示 -->
        <div class="image-capture-status" v-if="currentPhase === 'answer' && shouldCaptureImages()">
          <h4>📸 图像采集中</h4>
          <div class="capture-info">
            <p>已采集: {{ capturedImagesCount }}/10 张</p>
            <div class="capture-progress">
              <div class="progress-bar" :style="{ width: (capturedImagesCount / 10 * 100) + '%' }"></div>
            </div>
          </div>
        </div>
        
        <!-- 语音识别状态 -->
        <div class="speech-status" v-if="currentPhase === 'answer'">
          <h4>实时识别</h4>
...
请以JSON格式返回分析结果，格式如下：
{
  ...
  "recommendation": "综合建议"
    ...
    startInterview() {
      ...
      this.addMessage('interviewer', '你好，我是你今天的面试官。我们现在开始面试，请认真回答每个问题。');
    ...
    askNextQuestion() {
      ...
      this.addMessage('interviewer', `第${this.currentQuestionIndex + 1}题: ${question.question}`);
...
追问问题：`;
        ...
        this.addMessage('interviewer', `追问：${followUpQuestion}`);
        ...
        this.addMessage('interviewer', `追问：${defaultFollowUp}`);
        this.isFollowUpPhase = true;
    ...
    .user-panel {
      ...
      .question-status {
        ...
        p {
          ...
          .recording-indicator, .transcribing-indicator, .image-capture-indicator {
            margin-left: 8px;
